...
INSERT INTO `permission`(`permission_type`,`role_id`) VALUES ('whitelisteddevice','5');
...
ALTER TABLE admin_log MODIFY COLUMN activity_type VARCHAR(50) DEFAULT NULL;

ALTER TABLE da_admin_log CHANGE user_logon_id user_logon_id VARCHAR(100);

ALTER TABLE da_admin_log CHANGE admin_logon_id admin_logon_id VARCHAR(100);

/* Added by Abhimanyu for add column session_id to allow one session per user */
ALTER TABLE user_detail ADD COLUMN session_id VARCHAR(40) DEFAULT NULL;

/* Added by Puneet for Ipv6 related changes*/
...
ALTER TABLE `network_time_policy_mapping`  CHANGE `ipA` `ipA` VARCHAR(50),CHANGE `ipB` `ipB` VARCHAR(50);

ALTER TABLE `device_detail`  CHANGE `ip` `ip` VARCHAR(50);

ALTER TABLE `geo_city`  CHANGE `city_name` `city_name` VARCHAR(100);

ALTER TABLE logs_detail CHANGE ip ip VARCHAR(50);

ALTER TABLE ip_details_dashboard CHANGE ip ip VARCHAR(50);

ALTER TABLE ip_detail CHANGE ip_address ip_address VARCHAR(50), CHANGE last_ip_address last_ip_address VARCHAR(50);

ALTER TABLE log_report CHANGE ip_address ip_address VARCHAR(50);
