...
function openPolicy(){
	try{
		...
		$.ajax({
			type: "POST",  
			url: "policy_showAddModifyEAPPolicy.action",
			dataType: "text",
...
function save()
{
	...
	numberOfHours=  document.getElementById("numberOfHours").value;
	...
	else if(numberOfHours.length>5)
	{
		alert("Number of hours can not be more than 5 digits");
	}
	else if(isNaN(numberOfHours))
	{
		alert("Number of hours shoud be number");
	}
	else if(numberOfHours==0)
	{
		alert("Entered no of hours should not be 0");
	}
	else if(addTo==""){
		...
		if(addTo=="domain"){
			...
			else dataString='policyDesc='+policyDesc+"&numberOfHours="+numberOfHours+"&domainIds="+$.trim(domainIds)+"&csrfPreventionSalt="+strutsToken;
		...
		if(addTo=="App"){
			...
			if(appIds== null||appIds==""){
				...
				dataString='policyDesc='+policyDesc+"&numberOfHours="+$.trim(numberOfHours)+"&appIds="+$.trim(appIds)+"&csrfPreventionSalt="+strutsToken;
...
//		else 
		...
		try{
			$.ajax({
				type: "POST",
				url: "policy_createEAPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
...
function update()
{
	...
	else if(numberOfHours==0)
	{
		...
		dataString='policyDesc='+policyDesc+"&numberOfHours="+$.trim(numberOfHours)+"&pid="+$.trim(pid)+"&csrfPreventionSalt="+strutsToken;
...
//		else 
		{
			...
			$.ajax({
				type: "POST",
				url: "policy_updateEAPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
				success: function(response){
...
function delete_row()
{
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_deleteEAOPPolicy.action?pid="+$.trim(pid)+"&csrfPreventionSalt="+strutsToken,
			dataType: "text",
			success: function(response){
...
var res;
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_assignEAOPPolicy.action?pid="+$.trim(pid)+"&domainName="+$.trim(domainName)+"&appName="+$.trim(appName)+"&csrfPreventionSalt="+strutsToken,
			dataType: "text",
			success: function(response){
				if($.trim(response)=="sessionout"){
					testVal= document.getElementById('loginPage').value				
...
//	alert(domainIds);
	...
	myUrl+="?policyType=emergencyAuthenticationPolicy&domainIds="+$.trim(domainIds);
	}
	...
	myUrl+="?policyType=emergencyAuthenticationPolicy&appIds="+$.trim(appIds);
	}
	//alert(myUrl);
...
//end code for checkbox validation , Bug ID = 216 , added by abhimanyu
	...
	if(level=="domain"){
		var domainIds=$("#addto_domain_select").val();
		myUrl="policy_mapDomainWithEAPPolicy.action?domainIds="+$.trim(domainIds)+"&csrfPreventionSalt="+strutsToken;
		}
		if(level=="application"){
		var appIds=$("#addto_app_select").val();
		myUrl="policy_mapAppWithEAPPolicy.action?appIds="+$.trim(appIds)+"&csrfPreventionSalt="+strutsToken;
		}
...
function checkExistEdit(policyDesc) {
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_checkExistingEAPEdit.action?policyDesc="+policyDesc+"&policyDecBeforeEdit="+policyDecBeforeEdit,
			dataType: "text",
