							...
							{  
							    if(v.countryName == 'Unknown Network')
							      content += "<td> <a class='edit' onclick='showModelAddIpRange(\""+v.username+"\",\""+v.logReportId+"\");' href='#'>Add Ip range</td>";
									 ...
									 content += "<td> <a class='Apply' onclick='showModelAddCountry(\""+v.username+"\",\""+v.logReportId+"\");' href='#'>Apply Country</a>";
...
{
	$('.form-control.multiselect-search').val('').css("width", "75%");
	$('.filter-hidden').css("display", "block").css("class","active").removeClass("filter-hidden");
    ...
    $('.multiselect').attr('title', '');
       ...
       $('.multiselect').html('Select Country <b class="caret"></b>');
	...
	$('#selectCountryPolicyUser').val(userLogonId);
	$('#myModalAddIpRange').modal('show');
	 $('#myModalLabelMessage').html('Add IP Range');
return false;
}
...
if(ipRange1==""){
	alert("Please enter start IP range");
	return;
}
...
if(ipRange2==""){
	alert("Please enter end IP range");
	return;
}
	...
	{ 
		 ...
		 if($.trim(data)=="success"){
			// alert(data);
			 ...
			 $('#myModalLabelMessage').html('Apply Country Policy');
