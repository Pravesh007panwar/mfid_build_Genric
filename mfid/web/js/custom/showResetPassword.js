	...
	try {
		...
		$.ajax({
				...
				/*		content += '<td style="width:8%;">Status</td>';
						...
						content += '<td><input type="text" onkeydown="searchManageUser(event)"  id="mobile_mu" /><input type="hidden"id="addomain_mu" /><input type="hidden"id="userStatus_mu" /><input type="hidden"id="token_mu" /><input type="hidden" id="appName_mu" /> <input type="hidden" id="authType_mu" /> </td>';
						...
						content += '</select></td>';
						content += '<td><input type="hidden"id="token_mu" /></td>';*/
						/*
						...
						if (role != 4) {
							...
							content += '</div>';*/
							content += '<div class="btn-group">';
							content += '<button id="send_email_for_reset_password_button_id" type="button" class="btn btn-primary">Send Reset Password Email &nbsp;&nbsp; <i class="icon-envelope"></i></button>';
							content += '</div>';
						}
						content += '</div>';
						/*
...
function sendResetPasswordMail(userLogonIds) {
	// alert("deleteDomain=====");
	var strutsToken = $('[name=csrfPreventionSalt]').val();// added by puneet
															// vats
	...
	$.ajax({
		type : "POST",
		url : "admin_sendResetPasswordMail.action",
		data : dataString,
		dataType : "text",
		success : function(response) {
			// alert(response);
