...
var count;
var searchCount;
var searchShowResyncWithpageSize = false;
...
var globalShowResyncPreviouspageSize = "";
var globalShowResyncPreviousPageNum = "";
function openResyncDetail(pageCall, pageSearch) {
	try {

		if ($("#pageId_logreport").length) {
			globalShowResyncPreviouspageSize = $("#pageId_logreport").val();
			globalShowResyncPreviousPageNum = $("#pageNum_logreport").val();
		}
		var myUrl = "admin_showLogsData.action?logsType=resync";
		...
		if (searchShowResyncWithpageSize) {
			if (myUrl.indexOf('fetchSize') == -1) {
				if ($.trim(globalShowResyncPreviouspageSize) != '')
					myUrl += "&fetchSize=" + globalShowResyncPreviouspageSize;
			}
		}
		$('#user_resync_data').html("Loading...");
			$.ajax({
					...
					success : function(data) {
						...
						content += '<th><input type="text" onkeydown="searchResync(event)" name="userLogonId" id="userLogonId" ></th>';

						content += '<th><input type="text" onkeydown="searchResync(event)" name="lastSyncTime" id="lastSyncTime" ></th>';
						content += '<th><input type="text" onkeydown="searchResync(event)" name="role" id="role" ></th>';
						content += '</tr>';
						if (obj1 != null && obj1 != '') {
						...
						if ($.trim(globalShowResyncPreviouspageSize) != '') {
							$("#pageId_logreport").val(
									globalShowResyncPreviouspageSize);
							fetchSize_logreport();
							if (!searchShowResyncWithpageSize)
								$("#pageNum_logreport").val(
										globalShowResyncPreviousPageNum);
							else {
								if(globalShowResyncPreviousPageNum==0 && parseInt($('#pageN_logreport').text()) > 0)
									globalShowResyncPreviousPageNum="1";
								$("#pageNum_logreport").val(
										globalShowResyncPreviousPageNum);
							}
						}
						...
						if(pageCall==false){
							var size=10;
							if(globalShowResyncPreviouspageSize!=''){
								size = globalShowResyncPreviouspageSize;
								count = searchCount;
							}
							if(userLogonId!="" || requestTime!="" || role!=""){
								count = searchCount;
						...
						if(userLogonId=="" && requestTime=="" && role==""){
							searchShowResyncWithpageSize = false;
						}
					}
...
function search_data() {
	searchShowResyncWithpageSize = true;
	openResyncDetail(false, true);
}

function searchResync(e) {
	if (e.keyCode === 13)
		search_data();
}
