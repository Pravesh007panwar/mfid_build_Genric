...
var count;
var searchCount;
var searchShowRedisWithpageSize = false;

function restartRedisServer() {
	$.ajax({
		type : "POST",
		url : "admin_restartRedisServer.action",
		dataType : "text",
		success : function(response) {
			...
			openRedisServerDetail(false, false);
		}
...
function getPageData_logreport() {
	...
	if ($.trim(pageNumber) != "" && parseInt(pageNumber) <= parseInt(totalPages))
		openRedisServerDetail(true, false);
	else 
...
var globalShowRedisPreviouspageSize = "";
var globalShowRedisPreviousPageNum = "";
function openRedisServerDetail(pageCall, pageSearch) {
	try {

		if ($("#pageId_logreport").length) {
			globalShowRedisPreviouspageSize = $("#pageId_logreport").val();
			globalShowRedisPreviousPageNum = $("#pageNum_logreport").val();
		}
		var myUrl = "admin_showLogsData.action?logsType=redisRestart";
		...
		if (searchShowRedisWithpageSize) {
			if (myUrl.indexOf('fetchSize') == -1) {
				if ($.trim(globalShowRedisPreviouspageSize) != '')
					myUrl += "&fetchSize=" + globalShowRedisPreviouspageSize;
			}
		}
					...
					/*	content += '<select onChange="getPageData_logreport()" id="pageNum_logreport"  >';
						...
						if ($.trim(globalShowRedisPreviouspageSize) != '') {
							$("#pageId_logreport").val(
									globalShowRedisPreviouspageSize);
							fetchSize_logreport();
							if (!searchShowRedisWithpageSize)
								$("#pageNum_logreport").val(
										globalShowRedisPreviousPageNum);
							else {
								if(globalShowRedisPreviousPageNum==0 && parseInt($('#pageN_logreport').text()) > 0)
									globalShowRedisPreviousPageNum="1";
								$("#pageNum_logreport").val(
										globalShowRedisPreviousPageNum);
							}
						}
						//searchShowRedisWithpageSize = false;
						if(pageCall==false){
							var size=10;
							if(globalShowRedisPreviouspageSize!=''){
								size = globalShowRedisPreviouspageSize;
								count = searchCount;
							}
							if(userLogonId!="" || requestTime!="" || role!=""){
								count = searchCount;
...
function search_data() {
	searchShowRedisWithpageSize = true;
	openRedisServerDetail(false, true);
}
