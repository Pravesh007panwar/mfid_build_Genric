...
function insertGeoFileDetail(pageCall, pageSearch) {//	alert("123");
	try {
		...
		var myUrl = "admin_showLogsData.action?logsType=geoFile";
...
function searchIpRanges(pageCall, pageSearch) {
	...
	try {
		...
		var ipRangeVal = document.getElementById("ipRangeId").value;
		var myUrl = "admin_showIpRangeData.action?ipRangeVal=" + ipRangeVal;
		...
		$('#user_search_data_country_ipdata').html("Loading...");
			$.ajax({
					...
					success : function(data) {
						...
						var obj1 = JSON.parse(obj.geoIplocationList);
						var obj2 = JSON.parse(obj.count);
									...
									.each(
											...
											function(ind, val) {
												...
												content += "<tr><td><input type='checkbox' class='checkboxes' value='"
														+ val.geoIpListId+'|'+ val.startIpRange+'|'+ val.endIpRange
														+ "' /></td>";
...
function deleteIpRange(ids) {
	...
	if (ids == "") {
		...
		$.ajax({
			type : "POST",
			url : "admin_deleteIpRanges.action",
			data : dataString,
			dataType : "text",
...
function insertGeoFile() {
	...
	client.open("post", "admin_insertGeoFile.action", true);
...
function addIpAddress(countryName) {
	var res = "";
	var country = "";
	var countryListId = document.getElementById("countryListId").value;
...
//	}
	...
	try {
		...
		var myUrl = "admin_addCountryIPRange.action?countryName=" + country
				+ "&startIpRange=" + ipRange1 + "&endIpRange=" + ipRange2
				+ "&csrfPreventionSalt=" + strutsToken + "&geoIpListId="
				+ countryListId;
