...
function openPolicy(){
	try{
		...
		$.ajax({
			type: "POST",  
			url: "policy_showAddModifyTAPPolicy.action",
			dataType: "text",
...
{
	//alert("in function getapplication list");
	try{
...
function save()
{
	...
	{
		alert("Number of days shoud be number");
	}
	else if(numberOfDays==0)
	{
	...
	/*	var resExist=checkExist(policyDesc);
		...
		try{
			$.ajax({
				type: "POST",
				url: "policy_createTAPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
...
function update()
{
	...
	{
		alert("Description length should not be more than 50 character");
		return;
	}
	else if(numberOfDays=="")
	{
	...
	/*	var resExist=checkExistEdit(policyDesc);
		if(resExist.match(/exist/)){
			...
			$.ajax({
				type: "POST",
				url: "policy_updateTAPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
				success: function(response){
...
function delete_row()
{
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_deleteTAPPolicy.action?pid="+$.trim(pid)+'&csrfPreventionSalt='+strutsToken,
			dataType: "text",
			success: function(response){
...
var res;
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_assignTAPPolicy.action?pid="+$.trim(pid)+"&domainName="+$.trim(domainName)+"&appName="+$.trim(appName)+'&csrfPreventionSalt='+strutsToken,
			dataType: "text",
...
function addtoTable(){
	...
	myUrl+="?policyType=tokenactivationPolicy&domainIds="+$.trim(domainIds);
	}
	...
	myUrl+="?policyType=tokenactivationPolicy&appIds="+$.trim(appIds);
	}
...
function addPolicyToDomainApp() {
	...
	if(level=="domain"){
		...
		myUrl="policy_mapDomainWithTAPPolicy.action?domainIds="+$.trim(domainIds)+'&csrfPreventionSalt='+strutsToken;
		}
		if(level=="application"){
		var appIds=$("#addto_app_select").val();
		myUrl="policy_mapAppWithTAPPolicy.action?appIds="+$.trim(appIds)+'&csrfPreventionSalt='+strutsToken;
		}
...
function checkExist(policyDesc) {
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_checkExistingTAP.action?policyDesc="+policyDesc,
			dataType: "text",
...
function checkExistEdit(policyDesc) {
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_checkExistingTAPEdit.action?policyDesc="+policyDesc+"&policyDecBeforeEdit="+policyDecBeforeEdit,
			dataType: "text",
