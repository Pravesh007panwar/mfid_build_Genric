function openPolicy(){
	try{
		...
		$.ajax({
			type: "POST",  
			url: "policy_showAddModifyTRPPolicy.action",
			dataType: "text",
...
function save()
{
	...
	else if(numberOfDays==0)
	{
		...
		try{
			$.ajax({
				type: "POST",
				url: "policy_createTRPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
...
function update()
{
	...
	//	else {
			$.ajax({
				type: "POST",
				url: "policy_updateTRPPolicy.action",
				dataType: "text",
				data: dataString,
				async: false,
				success: function(response){
...
function delete_row()
{
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_deleteTRPPolicy.action?pid="+$.trim(pid)+'&csrfPreventionSalt='+strutsToken,
			dataType: "text",
			success: function(response){
...
var res;
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_assignTRPPolicy.action?pid="+$.trim(pid)+"&domainName="+$.trim(domainName)+"&appName="+$.trim(appName),
			dataType: "text",
...
function addtoTable(){
	...
	myUrl+="?policyType=tokenRevalidationPolicy&domainIds="+$.trim(domainIds);
	}
	...
	myUrl+="?policyType=tokenRevalidationPolicy&appIds="+$.trim(appIds);
	}
	$('#block_policy_addTo_data').html('<span>Loading...</span>');
		...
		$.ajax({
			...
			//	alert("data== "+data)
				...
				if(data!="" && data!=undefined){ // Added By Saurabh 
					var obj = JSON.parse(data);
				}
				
				if(obj!="" && obj!=undefined ){ // Added By Saurabh 
...
//end code for checkbox validation , Bug ID = 217 , added by abhimanyu
	...
	if(level=="domain"){
		var domainIds=$("#addto_domain_select").val();
		myUrl="policy_mapDomainWithTRPPolicy.action?domainIds="+$.trim(domainIds);
		}
		if(level=="application"){
		var appIds=$("#addto_app_select").val();
		myUrl="policy_mapAppWithTRPPolicy.action?appIds="+$.trim(appIds);
		}
...
function checkExist(policyDesc) {
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_checkExistingTRP.action?policyDesc="+policyDesc,
			dataType: "text",
...
function checkExistEdit(policyDesc) {
	...
	try{
		$.ajax({
			type: "POST",
			url: "policy_checkExistingTRPEdit.action?policyDesc="+policyDesc+"&policyDecBeforeEdit="+policyDecBeforeEdit,
			dataType: "text",
